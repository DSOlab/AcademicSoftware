...
def sat_at_emission_time(rsta, t_reception, interpolator, sat, tolerance=(.1, .1, .1)):
    ...
    while count_it < MAX_ITERATIONS:
        ...
        if np.any(np.greater_equal(np.abs(np.array((xnew, ynew, znew))-np.array((x,y,z))), np.array(tolerance))):
...
def sat_at_emission_time(rsta, t_reception, interpolator, tolerance=(.1, .1, .1)):
    """ Compute and return the satellite position at signal emission time.
        Parameters:
        rsta : Coordinates of observing station (ECEF, cartesian) in [m]
        t_reception: Time of reception, e.g. as recorded in RINEX
